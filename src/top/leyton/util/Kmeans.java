...
public class Kmeans {
    ...
    List<CheckinTable> checkinTableList = checkinUtil.getCheckinByUserId(22);
...
//        }
        ...
        temp[0] = rand.nextInt(size);
        center_old[0] = new Point();
        center_old[0].x = point[temp[0]].x;
        center_old[0].y = point[temp[0]].y;
        center_old[0].flag = 0;
        // 避免产生重复的中心
        for (int n = 1; n < count; n++) {
            int flage = 0;
            int thistemp = rand.nextInt(size);
            for (int j = 0; j < n; j++) {
                if (temp[j] == thistemp) {
                    flage = 1;// 有重复
                    break;
            ...
            if (flage == 1) {
                ...
                center_old[n] = new Point();
                center_old[n].x = point[thistemp].x;
                center_old[n].y = point[thistemp].y;
                center_old[n].flag = 0;// 0表示聚类中心
            }
